...
AI 생성 이미지를 활용해 상세페이지 레이아웃을 편집하는 캔버스 에디터.

### 기술

- react-konva / konva 사용
- 캔버스 라이브러리: Konva (Stage > Layer > Shape/Group 구조)

### 핵심 기능

1. 좌측 패널에서 레이아웃 템플릿 선택
2. 드래그앤드롭으로 캔버스에 배치
3. 배치된 템플릿 안에 이미지 삽입
4. 템플릿/이미지 이동, 크기 조절, 삭제
5. 완성된 상세페이지 이미지로 내보내기

### 자동화 기능 (백엔드 API 연동)

- 이미지 자동배치: 백엔드 API 응답으로 받은 이미지 위치/크기 데이터를 캔버스에 올바르게 렌더링
- 문구 자동생성: 백엔드 API 응답으로 받은 텍스트를 캔버스 텍스트 노드로 렌더링
- API 응답 구조 확정되면 AGENTS.md 업데이트 필요

### 캔버스 구조

- Stage: 전체 캔버스 영역
- Layer: 단일 레이어 사용
- 템플릿: Group으로 관리 (배경 Rect + 이미지 슬롯 + 텍스트 노드)
